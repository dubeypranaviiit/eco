...
import { getUnreadNotifications, markNotificationsAsRead } from '@/database/action';
import { createUser, getUserByEmail,getUserBalance } from '@/database/action';
...
const chainConfig = {
  ...
  rpcTarget: "https://goerli.infura.io/v3/YOUR_INFURA_PROJECT_ID", // RPC URL for Goerli
...
const clientId = process.env.WEB_3_AUTH_CLIENT_ID|| "BNy6-gxZvzi_gN0oe-6JtdGW14riP7ALYCp6D4MoMqVZQV8HOf_citMBjBAaXnHjrTuJ0gxngMfq-qXkYcCHuno";
...
const Header = ({onMenuClick,totalEarnings}:HeaderProps) => {
 ...
 const login = async ()=>{
  if(!web3auth){
    console.error(` Web3Auth is not initialised`);
    return ;
  }
  try{
...
try{
 ...
 const logout = async ()=>{
  if(!web3auth){
    console.log(`Web3Auth not found`);
    return;
  }
  try{
